...
(function () {
  ...
  function colorize(line, type) {
    if (type === "log" || type === "alert") {
      ...
      if (/WARN|DEFENSE|T-minus|ALERT|SOC|watchlist|INTEL/.test(line)) return '<span class="warn">' + escapeHtml(line) + "</span>";
    ...
    if (type === "shadow") {
      ...
      s = s.replace(/\b(bind|pierce|inject|spoof|loop|nullify|route|force_packet|encrypt|forge|mirror|desync|exfil|killswitch)\b/g, '<span class="func">$1</span>');
  ...
  function createWindow(cfg) {
    ...
    if (cfg.map) {
      el.innerHTML = titlePart +
        ...
        '<span class="node-compromised"></span>owned &nbsp;' +
        '<span class="node-scan"></span>scan &nbsp;' +
        '<span class="node-gca"></span>defense &nbsp;' +
        '<span class="node-safe"></span>clean' +
        "</div></div>";
    } else {
  ...
  async function runBootSequence() {
    ...
    for (var i = 0; i < SYSTEM_LOGS.length; i++) {
      ...
      if (item.text.indexOf("ShadowScript") !== -1 || item.text.indexOf("Engine") !== -1) {
  ...
  async function runMidSequence() {
    ...
    for (var i = 0; i < MID_SEQUENCE.length; i++) {
      ...
      if (item.progress) {
        await animateProgress("w-log", item.text, 1000);
        await sleep(100);
        continue;
      }
  ...
  function runSequence(winId) {
    ...
    function loop() {
      ...
      p.then(function () {
        if (w.lines.length > w.maxLines - 2 && Math.random() > 0.52) {
  ...
  function runAlertFlood() {
    ...
    function loop() {
      ...
      if (Math.random() > 0.8) {
        ...
        showToast("ALERT", short, /CRIT|Forced|escalated/.test(msg));
      }
      ...
      setTimeout(loop, 360 + Math.random() * 850);
    }
    loop();
  }
    ...
    function loop() {
      ...
      for (var i = 0; i < 3; i++) {
        ...
        if (lines[idx].indexOf("%") !== -1) {
          var pct = 45 + Math.floor(Math.random() * 50);
    ...
    function seed() {
      ...
      for (var i = 0; i < 24; i++) {
        nodes.push({
          ...
          type: i < 4 ? "gca" : (i < 13 ? "compromised" : (i < 18 ? "scan" : "safe")),
  ...
  function updateHud() {
    state.packets += Math.floor(Math.random() * 60) + 14;
    state.bytes += Math.floor(Math.random() * 20000) + 3000;
    if (Math.random() > 0.87) state.sessions = 2 + Math.floor(Math.random() * 10);
    if (state.phase === "free" && Math.random() > 0.93) state.threat = randomFrom(["LOW", "MED", "HIGH"]);
    updateHudImmediate();
  }
